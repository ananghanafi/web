    ...
    <v-layout column>
        <v-flex xs12>
            <v-card>
                ...
                <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="primary" flat @click.native="$router.push({name:'listorganisasi_daftar'})">Batal</v-btn>
...
export default {
    ...
    methods:{
        loadData(){
            return this.$store.dispatch('listorganisasi/daftar/show', parseInt(this.$route.params.id))
            .then(res=>{
                this.user = res
                this.person = res
