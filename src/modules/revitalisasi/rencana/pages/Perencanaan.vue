            ...
            <v-flex xs12>
                <v-card class="pb-3">
                    <v-card-title class="elevation-1 font-weight-bold">
                        PERENCANAAN KEGIATAN REVITALISASI
                    </v-card-title>
...
export default {
    ...
    methods: {
        ...
        load(pg=1){
            this.loading = true
            this.$store.dispatch('revitalisasi/perencanaan/get',{...this.filter,page:pg})
            .then(res=>{
                // let index = 0
                // ada pagging
        ...
        edit(item){
            this.$router.push({name:'revitalisasi_rencana_detail', params : {id : item.id}})
        },
        approve(item){
        ...
        updateItems(item){
            return this.$store.dispatch('revitalisasi/perencanaan/updateItems',{
