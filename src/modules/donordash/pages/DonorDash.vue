<template>
  <v-container grid-list-lg fill-height>
    <v-layout row wrap fill-height>
      <v-flex md12>DASHBOARD DONOR</v-flex>
          ...
          <v-layout row wrap>
            ...
            <v-flex xs4>
              <m-widget title="SEBARAN BIAYA DONOR BERDASARKAN MANDAT BRG">
                <v-data-table :hide-headers="true" :items="harga" class="elevation-1">
...
export default {
  ...
  methods: {
    load() {
      this.loading = true;
      this.$store.dispatch("donordash/getAnggaran").then(res=>{
          this.anggaran = res.anggaran;
      })
      this.$store.dispatch("perencanaan/getPlanningCost").then(res => {
        // this.anggaran = res.anggaran;
      });

      this.$store.dispatch("perencanaan/getPlanningArea").then(res => {
        this.totalArea = res.totalArea ? res.totalArea : [];
      });
      this.$store.dispatch("perencanaan/getPlanningAction").then(res => {
