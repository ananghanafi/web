                        ...
                        <v-tabs v-model="tab" fixed>
                                                    ...
                                                    <v-layout row wrap>
                                                        <v-flex md12>
                                                            <v-card color="secondary" tile>
                                                                <v-card-text>INFO KEGIATAN</v-card-text>
                                                            </v-card>
                                                        </v-flex>
                                                        <v-flex md4>
                                                            <v-text-field v-model="doc.year" label="TAHUN" disabled></v-text-field>
                                                        </v-flex>
                                                        <v-flex md4>                        
                                                            <v-text-field v-model="doc.code" label="KODE" disabled></v-text-field>
                                                        </v-flex>
                                                        <v-flex md4>
                                                            <v-text-field v-model="plan.status.status" label="STATUS" disabled></v-text-field>
                                                        </v-flex>
                                                        <v-flex md12>                        
                                            ...
                                            <v-layout v-if="doc" row wrap="">
                                                <v-flex md6>
                                                    ...
                                                    <v-layout row wrap="">
                                                        <v-flex md12>
                                                            <v-card color="secondary" tile>
                                                                <v-card-text>KOORDINAT GEOGRAFIS DAN AKSES LOKASI TITIK SUMUR BOR</v-card-text>
                                                            </v-card>
                                                        </v-flex>
                                                        <v-flex md12>
                                                            <v-layout row wrap="">
                            ...
                            <v-tab-item>
                                <v-card flat dense>
                                    ...
                                    <ProgressRealisasi module="skanal" :item="impl"/>                                
                                </v-card>
                            </v-tab-item>
                            <!-- Dokumentasi -->
                            <v-tab-item>
                                <Gallery :item="impl" module="skanal"/>
                            </v-tab-item>
                        </v-tabs>                    
                    </v-card-text>
                    <v-card-actions>
                        <v-spacer></v-spacer>
                        <v-btn color="primary" flat @click.native="$router.push({name:'skanal_implementasi'})">Batal</v-btn>
...
export default {
    ...
    methods:{
        editImg(tag) {
            ...
            switch (tag) {
                ...
                case 5:
                    ...
                    op.func = 'skanal/implementasi/uploadImage';
                    break;
                // case 6:
                //     op.img = null;
                //     op.func = 'skanal/implementasi/uploadGallery';
                //     op.options = {
                //         description: true,
                //         category: true,
                //     }
                ...
                case 7:
                    op.img = null;
                    op.func = 'skanal/implementasi/uploadDocument';
                    op.options = {
                        description: true,
                        category: false,
                        preview: false,
        ...
        loadData(){
            return this.$store.dispatch('skanal/implementasi/show', parseInt(this.$route.params.id))
            .then(res=>{
                this.impl = res
                ...
                this.list_canalType = [{id: this.doc.canalType.id, desc : this.doc.canalType.desc}]
                this.list_canalBlockingType = [{id: this.doc.canalBlockingType.id, desc : this.doc.canalBlockingType.desc}]
            })
        },        
        loadChoice(){
            return Promise.all([
            ...
            .then(([a,b, c, d, e, f, g])=>{
                this.list_canalBlockingType = a
                this.list_canalType = b
        ...
        save(){
            this.$store.dispatch('skanal/implementasi/update', this.impl)
            .then(()=>{
                this.$success('Data Implementasi berhasil disimpan')
                this.$router.push({name:'skanal_implementasi'})
            })
        }
    },
    computed:{
