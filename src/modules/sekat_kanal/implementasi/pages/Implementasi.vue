            ...
            <v-flex xs12>
                 <v-card class="pb-3">
                    <v-card-title class="elevation-1">
                        Pelaksanaan Pembangunan Sekat Kanal
                        <v-spacer></v-spacer>
...
export default {
    ...
    methods: {
        ...
        load(pg=1){
            this.loading = true
            this.$store.dispatch('skanal/implementasi/get',{...this.filter,page:pg})
            .then(res=>{
                // let index = 0
                // ada pagging
        ...
        edit(item){
            this.$router.push({name:'skanal_implementasi_detail', params : {id : item.id}})
        },
        progress(item){
            this.$router.push({name:'skanal_implementasi_progress', params : {id : item.id}})
        },        
        approve(item){
            ...
            .then(()=>{
                return this.$store.dispatch('skanal/implementasi/approveReject',{id:item.generalActivity.id, is_approve : true})
            })            
            .then(()=>{
                this.load()
            })
        ...
        reject(item){
            ...
            .then(()=>{
                return this.$store.dispatch('skanal/implementasi/approveReject',{id:item.generalActivity.id, is_approve : false})
            })            
            .then(()=>{
                this.load()
            })
